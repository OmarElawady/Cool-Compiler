...
void Feature_class::set_host_class(Symbol host)
{
   this->host_class = host;
}

Symbol Feature_class::get_host_class()
{
...
Expression method_class::get_expression()
{
   return expr;
}
...
Expression attr_class::get_expression()
{
   return init;
}
...
{
   return 1 + this->expr->calc_locals();
}
...
{
   int res = this->pred->calc_locals();
   max(res, this->then_exp->calc_locals());
   max(res, this->else_exp->calc_locals());
   return res;
}
...
int typcase_class::calc_locals()
{
   ...
   {
      branch_class *cs = (branch_class *)this->cases->nth(i);
      max(res, cs->calc_locals());
   }
   return res;
}
...
int block_class::calc_locals()
{
   ...
   {
      max(res, this->body->nth(i)->calc_locals());
   }
   return res;
}
...
{
   int res = this->init->calc_locals();
   max(res, 1 + this->body->calc_locals());
   return res;
}
...
{
   int res = this->e1->calc_locals();
   max(res, this->e2->calc_locals());
   return res;
}
